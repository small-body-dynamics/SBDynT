...
def prop_calc(j, astdys, datatype):
    print(j)
...
#    print(objname)
    ...
    try:
        ...
        fullfile = 'Sims/'+datatype+'/'+str(j)+'/archive_ias15.bin'
        print(fullfile)
        ...
        print(nump)
        if nump == 6:
            series = bin_to_df.bin_to_df(datatype,str(j),arc1,astdys,'4planet')
        elif nump == 10:
            series = bin_to_df.bin_to_df(datatype,str(j),arc1,astdys,'8planet')
...
if __name__ == '__main__':
    ...
    with MPIPool() as pool:
        ...
        if not pool.is_master():
            ...
            sys.exit(0)
        datatype = sys.argv[1]   
        astdys = pd.read_csv('data_files/'+datatype+'_data.csv')
        ...
        multi_prop = functools.partial(prop_calc, astdys=astdys, datatype=datatype)
        j = range(len(astdys))
        #j = range(22,23)
        #begin = datetime.now()

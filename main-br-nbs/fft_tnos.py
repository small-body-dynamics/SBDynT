...
for i in range(2):
    pumax[i] = np.max(pYpuc[1:])
    pnmax[i] = np.max(pYpnc[1:])
    pjmax[i] = np.max(pYpjc[1:])
    psmax[i] = np.max(pYpsc[1:])
    qumax[i] = np.max(pYquc[1:])
    qnmax[i] = np.max(pYqnc[1:])
    qjmax[i] = np.max(pYqjc[1:])
    qsmax[i] = np.max(pYqsc[1:])
    humax[i] = np.max(pYhuc[1:])
    hnmax[i] = np.max(pYhnc[1:])
    hjmax[i] = np.max(pYhjc[1:])
    hsmax[i] = np.max(pYhsc[1:])
    kumax[i] = np.max(pYkuc[1:])
    knmax[i] = np.max(pYknc[1:])

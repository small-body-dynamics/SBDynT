...
def setup_tno_integration_from_sv(des=None, clones=None, epoch = 2681000.0, sv = [0,0,0,0,0,0], cov = [], datadir='',save_sbdb=False,
                                  saveic=False,archivefile=None,logfile=False):
    '''
    ...
    if(clones==None):
        ...
        if(logf):
            ...
            logmessage += "a best-fit and 3-sigma minimum and maximum semimajor axis clones\n"
            tools.writelog(logf,logmessage)  
        iflag, epoch, sim, weights = run_reb.initialize_simulation_from_sv(planets=['outer'],
                          des=des, clones=clones, epoch = epoch, sv = sv, cov = cov, cloning_method= cloning_method,datadir=datadir,
                          logfile=logfile, save_sbdb=save_sbdb, saveic=saveic)
    else:
        cloning_method = 'Gaussian'
        iflag, epoch, sim = run_reb.initialize_simulation(planets=['outer'],

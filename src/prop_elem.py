...
######################################################################################################################                
    ...
    outputs =  np.array([np.nanmean(a_init),np.nanmean(e_init),np.nanmean(np.sin(inc_init)),np.nanmean(np.sqrt(a_init*(1-e_init**2))),np.nanmean(a_init),np.mean(ecc_f),np.nanmean(sini_f),np.nanmean(h_mom)])
    if debug == True:
        return outputs,Yhk_f,Ypq_f,g,s,freq1,freq2,spreads,freq_dist_lims

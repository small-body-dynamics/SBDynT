...
class TNO_ML_classifier:
    ...
    def __init__(
        self,
        G08_training_testing_data=None,
        G08_classifier_file=None,
        phi_training_testing_data=None,
        phi_classifier_file=None,
    ):
        ...
        if self.G08_classifier is None:
            ...
            if path.exists(self.G08_training_testing_data):
                ...
                try:
                    ...
                    print(
                        "TNO_ML_classifier.initialize_classifiers() failed when trying to fit the G08 classifier"
                    )
                    return 0
                try:
                ...
                if score < 0.95 and default:
                    ...
                    try:
                        ...
                        print(
                            "TNO_ML_classifier.initialize_classifiers() failed when trying to save the G08 classifier"
                        )
            else:
        ...
        if self.phi_classifier is None:
            ...
            if path.exists(self.phi_training_testing_data):
                ...
                try:
                    ...
                    print(
                        "TNO_ML_classifier.initialize_classifiers() failed when trying to fit the phi classifier"
                    )
                    return 0
                try:
                ...
                if score < 0.98 and default:
                    ...
                    try:
                        ...
                        print(
                            "TNO_ML_classifier.initialize_classifiers() failed when trying to save the phi classifier"
                        )
            else:
...
def calc_ML_features(
    ...
    if delta_nlong > 10 or delta_nshort > 10 or delta_length_long > 1e4 or delta_length_long > 5e2:
        ...
        logmessage += (
            f"{delta_nlong:.4f} {delta_nshort:.4f} {delta_length_long:.4f} {delta_length_short:.4f}\n\n"
        )
        ...
        if logfile != "screen":
            print(logmessage)
        return flag, None
    if delta_nlong > 0 or delta_nshort > 0 or delta_length_long > 5.0 or delta_length_long > 100.0:
    ...
    f.adot_min, f.adot_max, f.adot_mean, f.adot_stddev, f.adot_delta, junk, junk = basic_time_series_features(
        adot
    )
    f.edot_min, f.edot_max, f.edot_mean, f.edot_stddev, f.edot_delta, junk, junk = basic_time_series_features(
        edot
    )
    f.idot_min, f.idot_max, f.idot_mean, f.idot_stddev, f.idot_delta, junk, junk = basic_time_series_features(
        idot
    )
    f.qdot_min, f.qdot_max, f.qdot_mean, f.qdot_stddev, f.qdot_delta, junk, junk = basic_time_series_features(
        qdot
    )
    (
        f.Omdot_min,
    ...
    for n in range(0, clones + 1):
        ...
        (
            ...
            a_short[n], a_min_short[n], a_max_short[n], f.a_mean_short[n], f.a_stddev_short[n], delta=True
        )

        f.e_minmax_to_mean_density_ratio[n], f.e_min_to_max_density_ratio[n], junk, junk = histogram_features(
        ...
        (
            ...
            ec_short[n], f.e_min_short[n], f.e_max_short[n], f.e_mean_short[n], f.e_stddev_short[n]
        )

        f.i_minmax_to_mean_density_ratio[n], f.i_min_to_max_density_ratio[n], junk, junk = histogram_features(
        ...
        (
            ...
            inc_short, f.i_min_short[n], f.i_max_short[n], f.i_mean_short[n], f.i_stddev_short[n]
        )
...
def check_angle(img_clf, time, phi, max_prob=0.0, logfile=False):
    """
...
def rotating_frame_features(rh, phirf):
    ...
    for nr in bins:
        ...
        for n in range(0, int(nphbin)):
            rbinstd[nr] += (rph_count[nr, n] - rbinavg[nr]) * (rph_count[nr, n] - rbinavg[nr])
        if not (rbinavg[nr] == 0):
    ...
    for resorder in range(1, resorder_max + 1):
        rz_peri[resorder] = np.sqrt(
            sinphbar[0, resorder] * sinphbar[0, resorder] + cosphbar[0, resorder] * cosphbar[0, resorder]
        )
        rz_apo[resorder] = np.sqrt(
            sinphbar[-1, resorder] * sinphbar[-1, resorder] + cosphbar[-1, resorder] * cosphbar[-1, resorder]
        )

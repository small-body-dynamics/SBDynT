...
def add_planets(sim, planets=["all"], epoch=2459580.5, logfile="screen"):
    """
    inputs:
    ...
    if len(notplanets) > 0:
        ...
        for pl in notplanets:
            pflag, mass, radius, [x, y, z], [vx, vy, vz] = horizons_api.query_horizons_planets(
                obj=pl, epoch=epoch, logfile=logfile
            )
            if pflag < 1:
...
def initialize_simulation(
    ...
    sflag, epoch, sbx, sby, sbz, sbvx, sbvy, sbvz, weights = horizons_api.query_sb_from_jpl(
        des=des,
        clones=clones,
    ...
    # missing planets
    apflag, sim, sx, sy, sz, svx, svy, svz = add_planets(sim, planets=planets, epoch=epoch, logfile=logf)
    ...
    if clones > 0:
        for i in range(0, ntp):
            ...
            sbvy[i] += svy
            sbvz[i] += svz
            sim.add(m=0.0, x=sbx[i], y=sby[i], z=sbz[i], vx=sbvx[i], vy=sbvy[i], vz=sbvz[i], hash=sbhash)
    else:
        sbx += sx
        sby += sy
        sbz += sz
...
def initialize_simulation_at_epoch(
    ...
    flag, sbx, sby, sbz, sbvx, sbvy, sbvz = horizons_api.query_sb_from_horizons(
        des=des, epoch=epoch, logfile=logf
    )
    if flag < 1:
...
def initialize_simulation_from_sv(
    planets=["all"],
    des=None,
    clones=None,
    epoch=268100.0,
    ...
    sbx = np.concatenate(([x], sbx))
    sby = np.concatenate(([y], sby))
    sbz = np.concatenate(([z], sbz))
    sbvx = np.concatenate(([vx], sbvx))
    sbvy = np.concatenate(([vy], sbvy))
    sbvz = np.concatenate(([vz], sbvz))
...
def initialize_simulation_from_simarchive(des=None, archivefile=None, datadir="", logfile=False):
    """

...
from sbdynt import resonances
from sbdynt import machine_learning
...
class TNO_ML_outputs:
    ...
    def __init__(self, clones=0):
        ...
        self.res_img_probability = -1.0 * np.ones(self.clones + 1)
    ...
    def print_results(self):
        ...
        print(
            "Clone_number, most_probable_G08_class, p, q, m, n, phi_std_rad, phi_delta_rad, res_image_probability, probability_of_primary_class, ",
            end="",
        )
        ...
        for n in range(0, self.clones + 1):
            print(
                ...
                % (
                    n,
                    self.clone_classification[n],
                    self.res_p[n],
                    self.res_q[n],
                    self.res_m[n],
                    self.res_n[n],
                    self.res_phi_std[n],
                    self.res_phi_delta[n],
                    self.res_img_probability[n],
                    self.clone_confidence[n],
                ),
                end="",
            )
    ...
    def print_results_detailed(self):
        ...
        for n in range(0, self.clones + 1):
            ...
            for j in range(nclas):
                print("%e, " % self.class_probs[n][j], end="")
            print(
                "%e, %e, %e, " % (self.features.a_mean[n], self.features.e_mean[n], self.features.i_mean[n]),
                end="",
            )
            print(
                "%e, %e, %e, "
                % (self.features.a_stddev[n], self.features.e_stddev[n], self.features.i_stddev[n]),
                end="",
            )
            print("\n", end="")
...
def run_and_MLclassify_TNO(
    ...
    if sim == None and classify_only == False:
        ...
        iflag, sim, epoch, clones, cloning_method, weights = tno.setup_default_tno_integration(
            des=des, clones=clones, save_sbdb=sbdb_file, saveic=ic_file, archivefile=archivefile, logfile=logf
        )
        if iflag < 1:
    ...
    if not classify_only:
        ...
        rflag, sim = run_reb.run_simulation(
            sim, des=des, tmax=tmax, tout=50.0, archivefile=archivefile, deletefile=deletefile, logfile=logf
        )
        if rflag < 1:
    ...
    rflag, apl_short, ecpl, incpl, nodepl, peripl, mapl, tpl = tools.read_sa_by_hash(
    ...
    rflag, xr, yr, zr, vxr, vyr, vzr, tr = tools.calc_rotating_frame(
        des=des, planet="neptune", archivefile=archivefile, clones=clones, tmin=tmin, tmax=tmax
    )
    if rflag < 1:
    ...
    if a_short.all():
        ...
        tiss_short = apl_short / a_short + 2.0 * np.cos(inc_short) * np.sqrt(
            a_short / apl_short * (1.0 - ec_short * ec_short)
        )
    else:
        ...
        for n in range(0, clones + 1):
            if a_short[n].all():
                tiss_short[n] = apl_short / a_short[n] + 2.0 * np.cos(inc_short[n]) * np.sqrt(
                    a_short[n] / apl_short * (1.0 - ec_short[n] * ec_short[n])
                )
    ...
    all_classified = 1
    for n in range(0, clones + 1):
        if a_short[n, 0] < 28.0 or a_mean[n] < 29:
    ...
    if not classify_only:
        ...
        if logf:
            ...
            tools.writelog(logf, logmessage)
        rflag, sim = run_reb.run_simulation(
            sim, des=des, tmax=tmax, tout=1000.0, archivefile=archivefile, deletefile=False, logfile=logf
        )
        if rflag < 1:
    ...
    if a.all():
        ...
        for n in range(0, clones + 1):
            if a[n].all():
                tiss[n] = apl / a[n] + 2.0 * np.cos(inc[n]) * np.sqrt(a[n] / apl * (1.0 - ec[n] * ec[n]))
    ...
    fflag, tno_class.features = machine_learning.calc_ML_features(
        t_long,
        a_long,
        ec_long,
        inc_long,
    ...
    for n in range(tno_class.clones + 1):
        ...
        if predicted_class == "not_TNO":
            ...
            (
                ...
                delta_phi_id,
                phi,
            ) = run_res_angle_classifier(
                img_clf=clf.phi_classifier,
                time=t_long,
                lambda_pl=lambda_pl_long,
                lambda_tp=lambda_long[n],
                node=node_long[n],
                pomega=pomega_long[n],
                apl=a_pl_long,
                a=a_long[n],
            ...
            if (
                tno_class.clone_classification[n] == "Nresonant"
                and tno_class.features.a_delta[n] > 2.5
    ...
    if related_clones == True:
        for n in range(tno_class.clones + 1):
            if (
                tno_class.clone_classification[n] != "Nresonant"
                and tno_class.features.a_delta[n] < 2.5
                and res_check_performed[n] < 1.0
            ):
                for i in range(1, len(pres)):
                    phi = (
                        pres[i] * lambda_long[n]
                        - qres[i] * lambda_pl_long
                        - mres[i] * pomega_long[n]
                        - nres[i] * node_long[n]
                    )
                    cflag, angle_prob, sigma_phi, delta_phi = machine_learning.check_angle(
                        clf.phi_classifier, t_long, phi, -1.0, logfile=logf
                    )
...
def run_res_angle_classifier(
    ...
    denom = np.array([1, 1])
    prtol = 0.02
    prmax = 1.0 / ((1.0 - prtol) * min(obspr, obspr2, obspr1))
    prmin = 1.0 / ((1.0 + prtol) * max(obspr, obspr2, obspr1))
    ...
    if prmax > 1.0 and prmin < 1.0:
        ...
        cflag, prob, sigma_phi, delta_phi = machine_learning.check_angle(
            img_clf, time, phi, max_prob, logfile=logf
        )
        if not (cflag):
    ...
    while res_flag == 0:
        ...
        ftflag, num, denom, new_check_q, new_check_p, n_check = resonances.farey_tree(
        ...
        for jk in range(n_check):
            ...
            if mr_e >= 2:
                ...
                if (prob > 0.5 and res_flag == 0) or (prob > max_prob and res_flag == 1):
                    ...
                    prob == max_prob
                    and sigma_phi < sigma_phi_id
                    and (
                        (delta_phi_id > 355 * np.pi / 180.0 and delta_phi < 355 * np.pi / 180.0)
                        or delta_phi < 0.9 * delta_phi_id
                    )
                ):
                    if prob > 0.5:
                        res_flag = 2
            ...
            if flag != 1 and mr_e >= 4 and incbar > 7.0 * np.pi / 180.0:
                mr = mr_e - 4
                nr = 4
                while mr >= 0:
                    phi = pr * lambda_tp - qr * lambda_pl - mr * pomega - nr * node
                    ...
                    mr = mr - 2
                    nr = nr + 2
                    if nr > 8 and incbar < 30.0 * np.pi / 180:

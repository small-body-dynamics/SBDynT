...
# internal modules
from sbdynt import plotting_scripts
...
class proper_element_class:
    ...
    def __init__(self, des="", clones=0):
        ...
        self.osculating_elements = {
            ...
            "e": np.empty(clones + 1),
            "I": np.empty(clones + 1),
            "omega": np.empty(clones + 1),
            "Omega": np.empty(clones + 1),
        }
        self.proper_elements = {
            "a": np.zeros(clones + 1),
            "e": np.zeros(clones + 1),
            "sinI": np.zeros(clones + 1),
            'g("/yr)': np.zeros(clones + 1),
            's("/yr)': np.zeros(clones + 1),
            "g(rev/yr)": np.zeros(clones + 1),
            "s(rev/yr)": np.zeros(clones + 1),
            "omega": np.zeros(clones + 1),
            "Omega": np.zeros(clones + 1),
        }
        self.mean_elements = {
            "a": np.empty(clones + 1),
            "e": np.empty(clones + 1),
            "sinI": np.empty(clones + 1),
            'g("/yr)': np.empty(clones + 1),
            's("/yr)': np.empty(clones + 1),
            "g(rev/yr)": np.empty(clones + 1),
            "s(rev/yr)": np.empty(clones + 1),
        }
        self.proper_errors = {
            "RMS_a": np.empty(clones + 1),
            "RMS_e": np.empty(clones + 1),
            "RMS_sinI": np.empty(clones + 1),
            'RMS_g("/yr)': np.empty(clones + 1),
    ...
    def print_results(self):
        print(
            str(self.des),
            " Proper Element Results from a ",
            str(round(self.tmax / 1e6)),
            " Myr integration with outputs every",
            str(round(self.tout)),
            "years",
        )
        ...
        print(
            "%-22s %10.6f  %10.6f  %10.6f   %10.6f   %10.6f"
            % (
                "Mean Elements",
        ...
        print(
            "%-22s %10.2e  %10.2e  %10.2e   %10.2e   %10.2e"
            % (
                "Proper Errors",
        ...
        if self.clones > 0:
            ...
            for i in range(self.clones):
                print(
                    "Clone %3d %12s %10.6f  %10.6f  %10.6f   %10.6f   %10.6f"
                    % (
                        i,
                        "",
            ...
            print(
                "%-22s %10.2e  %10.2e  %10.2e   %10.2e   %10.2e"
                % ("RMS(Clones+Best-fit)", rmsa, rmse, rmsi, rmsg, rmss)
            )
        ...
        if self.scattered["scattered"]:
            print(
                str(self.des)
                + " may have been scattered during the simulation at t="
                + str(self.scattered["scat_time"])
                + " years, near snapshot #"
                + str(self.scattered["scat_ind"])
            )

        if self.family_results["family_name"] != None:
            print(
                str(self.des)
                + " maintains a proper orbit which corresponds dynamically to the "
                + self.family_results["family_name"]
                + " family."
            )
            print(
                "The relative velocity of this object with respect to the family collisional center = "
                + str(round(self.family_results["pairwise_dMet"]), 2)
                + "m/s"
            )
...
def integrate_for_pe(
    ...
    if direction == "bf":
        ...
        iflag, snew, clones = run_reb.initialize_simulation_from_simarchive(
            des=des, archivefile=ic_file, logfile=logf
        )
        if iflag < 1:
        ...
        rflag, snew = run_reb.run_simulation(
            snew,
            des=des,
            archivefile=archivefile,
            logfile=logf,
...
def smooth_fft_convolution(
    fft_signal,
    freqs,
    primary_freqs,
    time,
    protect_radius_bins_init=3,
    kernel_size=15,
    method="gaussian",
    inc_filt=False,
    known_planet_freqs=[],
    ...
    scaling_factor = np.sqrt(
        1 / ((np.abs(smoothed_fft[f_pf_idx]) ** 2) / (np.abs(fft_signal[f_pf_idx]) ** 2))
    )
...
def extract_proper_mode(
    ...
    try:
        ...
        if filt_time:
            filt_signal = smooth_fft_time(
                spectrum,
                freqs,
                proper_freq,
                time,
                protect_radius_bins_init=protect_bins,
                kernel_size=kernel,
                method="gaussian",
                inc_filt=inc_filt,
                known_planet_freqs=known_planet_freqs,
                freq_tol=freq_tol,
                win=win,
                shortfilt=shortfilt,
            )
        else:
            filt_signal = smooth_fft_convolution(
                spectrum,
                freqs,
                proper_freq,
                time,
...
def find_local_max_windowed(freqs, powers, window_half_dex=0.05, window_protect_dex=0.1):
    ...
    dominant_freq = np.sum(freqs[ind_low:ind_high] * (powers[ind_low:ind_high]) ** 2) / np.sum(
        (powers[ind_low:ind_high]) ** 2
    )
...
def get_planet_freqs(t_init, planet_elems, small_planets_flag=False):
    ...
    try:
        ...
        max_idx, g5, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        g_arr.append(g5)
        ...
        max_idx, g6, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        g_arr.append(g6)
        g_inds.append(max_idx)
        ...
        powers[0] = 0
        powers = ind_filt(g_inds, powers, freq, dexl, div_num)
        max_idx, g7, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        g_arr.append(g7)
        g_inds.append(max_idx)
        ...
        powers[0] = 0
        powers = ind_filt(g_inds, powers, freq, dexl, div_num)
        max_idx, g8, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        g_arr.append(g8)
        g_inds.append(max_idx)
        ...
        powers[bad_freq_inds] = 0
        max_idx, s6, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        s_arr.append(s6)
        ...
        max_idx, s7, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        s_arr.append(s7)
        s_inds.append(max_idx)
        ...
        powers[bad_freq_inds] = 0
        powers = ind_filt(s_inds, powers, freq, dexl, div_num)
        max_idx, s8, local_power_all, protect_bins = find_local_max_windowed(
            freq, powers, window_half_dex=dex, window_protect_dex=0.15
        )
        s_arr.append(s8)
        s_inds.append(max_idx)
        ...
        if small_planets_flag:
            ...
            max_idx, g2, local_power_all, protect_bins = find_local_max_windowed(
                freq, powers, window_half_dex=dex, window_protect_dex=0.15
            )
            g_arr.append(g2)
            g_inds.append(max_idx)
            ...
            powers[bad_freq_inds] = 0
            powers = ind_filt(g_inds, powers, freq, dexl, div_num)
            max_idx, g3, local_power_all, protect_bins = find_local_max_windowed(
                freq, powers, window_half_dex=dex, window_protect_dex=0.15
            )
            g_arr.append(g3)
            g_inds.append(max_idx)
            ...
            powers[bad_freq_inds] = 0
            powers = ind_filt(g_inds, powers, freq, dexl, div_num)
            max_idx, g4, local_power_all, protect_bins = find_local_max_windowed(
                freq, powers, window_half_dex=dex, window_protect_dex=0.15
            )
            g_arr.append(g4)
            g_inds.append(max_idx)
            ...
            powers[bad_freq_inds] = 0
            powers = ind_filt(s_inds, powers, freq, dexl, div_num)
            max_idx, s3, local_power_all, protect_bins = find_local_max_windowed(
                freq, powers, window_half_dex=dex, window_protect_dex=0.15
            )
            s_arr.append(s3)
            s_inds.append(max_idx)
            ...
            powers[bad_freq_inds] = 0
            powers = ind_filt(s_inds, powers, freq, dexl, div_num)
            max_idx, s4, local_power_all, protect_bins = find_local_max_windowed(
                freq, powers, window_half_dex=dex, window_protect_dex=0.15
            )
            s_arr.append(s4)
            s_inds.append(max_idx)
            ...
            powers[bad_freq_inds] = 0
            powers = ind_filt(s_inds, powers, freq, dexl, div_num)
            max_idx, s2, local_power_all, protect_bins = find_local_max_windowed(
                freq, powers, window_half_dex=dex, window_protect_dex=0.15
            )
            s_arr.append(s2)
            s_inds.append(max_idx)
...
def compute_prop(
    ...
    try:
        ...
        try:
            ...
            g_idx, g, local_power_all, protect_g_bins = find_local_max_windowed(
                freq_s, power, window_half_dex=0.05, window_protect_dex=0.15
            )
            if abs(1 / g) > 5e5:
            ...
            s_idx, s, local_power_all, protect_s_bins = find_local_max_windowed(
                freq_s, power, window_half_dex=0.02, window_protect_dex=0.15
            )
            if abs(1 / s) > 5e5:
    ...
    try:
        ...
        hk_old, hk_freq, protect_hk, hk_signal = extract_proper_mode(
            hk_arr,
            t_init,
            g_arr,
            freq_tol=tol,
            kernel=kernel_g,
            protect_bins=protect_g_bins,
            proper_freq=protect_g,
            shortfilt=shortfilt,
        )
        pq_old, pq_freq, protect_pq, pq_signal = extract_proper_mode(
            pq_arr,
            t_init,
            s_arr,
            freq_tol=tol,
            kernel=kernel_s,
            proper_freq=protect_s,
            protect_bins=protect_s_bins,
            shortfilt=shortfilt,
        )
        ...
        ee_new, ee_freq, protect_ee, ee_signal = extract_proper_mode(
            ee_old,
            t_init,
            g_arr,
            freq_tol=tol,
            kernel=kernel_g * 2,
            protect_bins=0,
            proper_freq=np.append(protect_g[0], [0]),
            shortfilt=False,
            filt_time=True,
        )
        II_new, II_freq, protect_II, II_signal = extract_proper_mode(
            II_old,
            t_init,
            s_arr,
            freq_tol=tol,
            kernel=kernel_s * 2,
            proper_freq=np.append(protect_s[0], [0]),
            protect_bins=0,
            shortfilt=False,
            filt_time=True,
        )
        ...
        max_idx_g, g_fin, local_power_all, protect_bins = find_local_max_windowed(
            freq, np.abs(np.fft.fft(hk_new)) ** 2, window_half_dex=0.02, window_protect_dex=0.15
        )
        max_idx_s, s_fin, local_power_all, protect_bins = find_local_max_windowed(
            freq, np.abs(np.fft.fft(pq_new)) ** 2, window_half_dex=0.02, window_protect_dex=0.15
        )
        ...
        a_filt, a_freq, protect_a, a_signal = extract_proper_mode(
            a_init,
            t_init,
            g_arr,
            freq_tol=tol,
    ...
    for j in range(windows):
        ...
        try:
            ...
            g_idx_e, g_e, local_power_all_e, protect_g_bins_e = find_local_max_windowed(
                freqe, power_e, window_half_dex=0.05, window_protect_dex=0.15
            )
            s_idx_e, s_e, local_power_all_e, protect_s_bins_e = find_local_max_windowed(
                freqe, power_I, window_half_dex=0.05, window_protect_dex=0.15
            )
            ...
            hk_newe, hk_freqe, protect_hk_waste, hk_waste = extract_proper_mode(
                hk_in,
                t_input,
                g_arr,
                freq_tol=tol,
                ...
                kernel=kernele_g,
                proper_freq=hk_freqe,
                win=True,
            )
            pq_newe, pq_freqe, protect_pq_waste, hk_waste = extract_proper_mode(
                pq_in,
                t_input,
                s_arr,
                freq_tol=tol,
        ...
        ee_newe, ee_freqe, protect_eee, ee_signale = extract_proper_mode(
            ee_olde,
            t_input,
            g_arr,
            freq_tol=tol,
        ...
        II_newe, II_freqe, protect_IIe, II_signale = extract_proper_mode(
            II_olde,
            t_input,
            s_arr,
            freq_tol=tol,
        ...
        a_filtn, a_freqn, protect_an, a_signaln = extract_proper_mode(
            a_input,
            t_input,
            g_arr,
            freq_tol=tol,
    ...
    if rms:
        rms_val = np.sqrt(
            np.nanmean(
                (np.array(error_list, dtype=np.float64) - np.array(pes, dtype=np.float64)) ** 2, axis=0
            )
        )
    else:
...
def hcm_pair(a1, a2, e1, e2, sini1, sini2):
    ...
    return (
        ...
        * am
        * 1.498e11
        * np.sqrt(5 / 4 * ((a2 - a1) / am) ** 2 + 2 * (e2 - e1) ** 2 + 2 * (sini2 - sini1) ** 2)
    )
...
def calc_proper_elements(
    ...
    if len(planet_elems) == 0:
        ...
        if gs_dict == None:
            logmessage = (
                "Neither orbital element arrays for the planets nor a gs_dict were provided by the user.\n"
            )
        ...
        g_arr, g_inds, s_arr, s_inds, gs_dict = get_planet_freqs(
            times, planet_elems, small_planets_flag=small_planets_flag
        )
    ...
    proper_object.proper_indicators["Distance Metric"] = hcm_calc(
        prop_elem["a"][0], prop_errs["RMS_a"][0], prop_errs["RMS_e"][0], prop_errs["RMS_sinI"][0]
    )
    ...
    for key, val in proper_object.secular_frequencies.items():
        ...
        proper_object.secfreq_flags[key] = (
            abs(val * 3600 * 360) < 0.01,
            abs(val * 3600 * 360) < 0.05,
            abs(val * 3600 * 360) < 0.1,
            abs(val * 3600 * 360) < 0.2,
        )
...
def check_family_candidates(proper_object):
    ...
    for i in range(len(fam_df)):
        ...
        hcm_cen = hcm_pair(
            ...
            fam_obj["cen_e"],
            prope["e"][0],
            np.sin(fam_obj["cen_I"] / 180 * np.pi),
            prope["sinI"][0],
        )

        if (
            (hcm_cen < fam_obj["hcm_cut"])
            and (prope["a"][0] > fam_obj["low_a"])
            and (prope["a"][0] < fam_obj["high_a"])
            and (prope["e"][0] > fam_obj["low_e"])
            and (prope["e"][0] < fam_obj["high_e"])
            and (prope["sinI"][0] > np.sin(np.pi / 180 * fam_obj["low_I"]))
            and (prope["sinI"][0] < np.sin(np.pi / 180 * fam_obj["high_I"]))
        ):

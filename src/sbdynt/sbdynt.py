...
warnings.filterwarnings(
    "ignore", message="File in use for Simulationarchive already exists. Snapshots will be appended."
)
...
def run_ast(
    ...
    iflag, sim, epoch, clones, cloning_method, weights = setup_default_ast_integration(
        des=des,
        clones=clones,
        save_sbdb=ast_results.sbdb_file,
        planets=ast_results.planets,
        saveic=ast_results.icfile,
    ...
    if run_proper:
        ...
        rflag, sim = integrate_for_pe(
            sim,
            des=des,
            archivefile=ast_results.archivefile,
            icfile=ast_results.icfile,
            logfile=ast_results.logfile,
            tmax=10e6,
            tout=500.0,
            direction="bf",
        ...
        if reflag < 1:
            logmessage = (
                "Failed in sbdynt.run_ast when reading in integrated simulation for proper elements\n"
            )
        ...
        pflag, prope = calc_proper_elements(
            des=des,
            times=times,
            sb_elems=sb_elems,
    ...
    if run_stability:
        ...
        st_flag, ast_results.stability_indicators = compute_stability(
            des=des,
            times=times,
            sb_elems=sb_elems,
            clones=ast_results.clones,
...
def analyze_ast_run(
    ...
    if clones == None:
        ...
        iflag, snew, clones = initialize_simulation_from_simarchive(
            des=des, archivefile=ast_results.archivefile, logfile=logf
        )
        if iflag < 1:
    ...
    if run_stability:
        if logf:
            ...
            writelog(logf, logmessage)
        stflag, ast_results.stability_indicators = compute_stability(
            des=des,
            times=times,
            sb_elems=sb_elems,
            clones=clones,
...
def run_tno(
    ...
    if run_proper or run_stability:
        ...
        rflag, sim = integrate_for_pe(
            ...
            des=des,
            archivefile=tno_results.archivefile,
            icfile=tno_results.icfile,
            logfile=tno_results.logfile,
            tmax=150e6,
            tout=5000.0,
            direction="bf",
            deletefile=pe_deletefile,
            integrator=integrator,
        )
        if rflag < 1:
        ...
        reflag, times, sb_elems, planet_elems, clone_elems, small_planets_flag = read_archive_for_pe(
            ...
            clones=tno_results.clones,
            logfile=tno_results.logfile,
            object_type=tno_results.object_type,
        )
        if reflag < 1:
            logmessage = (
                "Failed when reading in the integrated simulation for proper elements in sbdynt.run_tno\n"
            )
            logmessage += "at prop_elem.read_archive_for_pe\n"
            writelog(logf, logmessage)
            if logf != "screen":
        ...
        if run_stability:
            stflag, tno_results.stability_indicators = compute_stability(
                des=des,
                times=times,
                sb_elems=sb_elems,
                clones=tno_results.clones,
...
def analyze_tno_run(
    ...
    if clones == None:
        ...
        if logf:
            ...
            writelog(logf, logmessage)
        iflag, snew, clones = initialize_simulation_from_simarchive(
            des=des, archivefile=tno_results.archivefile, logfile=logf
        )
        if iflag < 1:
...
def run_sb(
    ...
    iflag = 0
    if object_type == "ast":
        iflag, sim, epoch, clones, cloning_method, weights = setup_ast_integration(
            des=des,
    ...
    if sb_results.int_direction == "forward":
        rflag, sim = run_simulation(
            ...
            datadir=datadir,
            logfile=logfile,
            tmax=sb_results.tmax,
            tout=sb_results.tout,
        )
        if rflag < 1:
    ...
    pflag, pe, sb_elems = calc_proper_elements(
        des=des,
        datadir=datadir,
        archivefile=archivefile,
        clones=clones,
    ...
    sb_results.stability_indicators = compute_stability(
        des=des,
        times=times,
        sb_elems=sb_elems,
        clones=clones,
...
def run_existing_sb(
    ...
    if run_stability:
        ...
        tno_results.stability_indicators = compute_stability(
            des=des,
            times=times,
            sb_elems=sb_elems,
            clones=clones,

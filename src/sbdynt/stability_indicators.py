...
class stability_indicators:
    ...
    def print_results(self):
        ...
        if self.ACFI != None:
            ...
            print(
                "ACFI: (",
                self.ACFI < self.flag_limits["ACFI"],
                "), ",
                self.ACFI,
                sign,
                self.flag_limits["ACFI"],
            )
        else:
        ...
        if self.Entropy != None:
            ...
            print(
                "Entropy: (",
                self.Entropy < self.flag_limits["Entropy"],
                "), ",
                self.Entropy,
                sign,
        ...
        if self.Power != None:
            ...
            print(
                "Power: (",
                self.Power < self.flag_limits["Power"],
                "), ",
                self.Power,
                sign,
        ...
        if self.Distance_metric != None:
            if self.Distance_metric < self.flag_limits["Distance_metric"][0]:
                print(
                    "Distance Metric: Stable,",
                    self.Distance_metric,
                    " < ",
                    self.flag_limits["Distance_metric"][0],
                    "m/s",
                )
            elif self.Distance_metric < self.flag_limits["Distance_metric"][1]:
                print(
                    "Distance Metric: Metastable,",
                    self.flag_limits["Distance_metric"][0],
                    " < ",
                    self.Distance_metric,
                    " < ",
                    self.flag_limits["Distance_metric"][1],
                    "m/s",
                )
            elif self.Distance_metric > self.flag_limits["Distance_metric"][1]:
                print(
                    "Distance Metric: Unstable,",
                    self.Distance_metric,
                    " > ",
                    self.flag_limits["Distance_metric"][1],
                    "m/s",
        ...
        if self.Clone_RMS_a != None:
            ...
            print(
                "Clone_RMS_a: (",
                self.Clone_RMS_a > self.flag_limits["Clone_RMS_a"],
                "), ",
                self.Clone_RMS_a,
                sign,
                self.flag_limits["Clone_RMS_a"],
            )
        ...
        if self.Clone_RMS_e != None:
            ...
            print(
                "Clone_RMS_e: (",
                self.Clone_RMS_e > self.flag_limits["Clone_RMS_e"],
                "), ",
                self.Clone_RMS_e,
                sign,
                self.flag_limits["Clone_RMS_e"],
            )
        ...
        if self.Clone_RMS_sinI != None:
            ...
            print(
                "Clone_RMS_sinI: (",
                self.Clone_RMS_sinI > self.flag_limits["Clone_RMS_sinI"],
                "), ",
                self.Clone_RMS_sinI,
                sign,
                self.flag_limits["Clone_RMS_sinI"],
            )
...
def compute_stability(
    des=None, times=[], sb_elems=[], clones=0, pe_obj=None, clone_elems=[], output_arrays=False, logfile=False
):
    """
    ...
    if len(sb_elems) == 0:
        logmessage = (
            "The sb_elems variable is empty. Call the function again with the sb_elems variable defined\n"
        )
    ...
    try:
                ...
                if len(clone_elems) == 0:
                    ...
                    for i in range(clones):
                        if i >= len(clone_elems):
                            ...
                            logmessage += (
                                "which is " + str(len(clone_elems)) + "Stopping iterating over clone_elems\n"
                            )
                            tools.writelog(logfile, logmessage)
                            break
        ...
        if pe_obj != None:
            if pe_obj.proper_elements["a"][0] > 0:
                try:
                    ci.Distance_metric = prop_elem.hcm_calc(
                        pe_obj.proper_elements["a"][0],
                        pe_obj.proper_errors["RMS_a"][0],
                        pe_obj.proper_errors["RMS_e"][0],
                        pe_obj.proper_errors["RMS_sinI"][0],
                    )
                except Exception as err:
                    logmessage = str(err)
                    logmessage += (
                        "-> pe_obj.proper_elements and/or pe_obj.proper_errors does not contain valid\n"
                    )
...
def power_prop_calc(t, e, I, omega, Omega, size=3, pe_obj=None):
    ...
    g_idx, g, local_power_all, protect_g_bins = prop_elem.find_local_max_windowed(
        freq_s, power_hk, window_half_dex=0.02, window_protect_dex=0.15
    )
    ...
    s_idx, s, local_power_all, protect_s_bins = prop_elem.find_local_max_windowed(
        freq_s, power_pq, window_half_dex=0.02, window_protect_dex=0.15
    )

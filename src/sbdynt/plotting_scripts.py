...
from sbdynt import stability_indicators as si
    ...
    if des == None:
        ...
        try:
            ...
            rflag, a, e, inc, node, aperi, ma, t = tools.read_sa_for_sbody(
                des=des, datadir=datadir, archivefile=archivefile, tmax=tmax, tmin=tmin, clones=clones
            )
            if rflag < 1:
...
def calc_and_plot_rotating_frame(
    ...
    rflag, xr, yr, zr, vxr, vyr, vzr, t = tools.calc_rotating_frame(
        des=des, archivefile=archivefile, planet=planet, clones=clones, tmin=tmin, tmax=tmax
    )
    ...
    rflag, pxr, pyr, pzr, pvxr, pvyr, pvzr, t = tools.calc_rotating_frame(
        des=planet, archivefile=archivefile, planet=planet, clones=0, tmin=tmin, tmax=tmax
    )
    ...
    plt.subplots_adjust(left=None, bottom=None, right=None, top=0.92, wspace=0.35, hspace=0.35)
...
def plot_osc_and_prop(prop_elem):
    ...
    ax[1].hlines(
        np.mean(np.abs(hk)), xmin=t[0], xmax=t[-1], colors="r", label="Mean Element", ls="--", alpha=0.75
    )
    ax[2].hlines(
        np.mean(np.arcsin(np.abs(pq))) * 180 / np.pi, xmin=t[0], xmax=t[-1], colors="r", ls="--", alpha=0.75
    )
...
def plot_power(ci, pe_obj=None):
    ...
    top_hk = np.sum(Yhk[gind - 5 : gind + 6])
    top_pq = np.sum(Ypq[sind - 5 : sind + 6])
...
def plot_freq_space(prop_elem, ifreqs={}):
    ...
    if prop_elem.p_eI:
        ...
        ax[0].axvline(
            abs(1 / (prop_elem.proper_elements["g(rev/yr)"][0] - pf["g5"])),
            c="r",
            ls="--",
            alpha=alp,
            label="g-g5",
        ...
        ax[0].axvline(
            abs(1 / (prop_elem.proper_elements["g(rev/yr)"][0] - pf["g6"])),
            c="goldenrod",
            ls="--",
            alpha=alp,
            label="g-g6",
        ...
        ax[0].axvline(
            abs(1 / (prop_elem.proper_elements["g(rev/yr)"][0] - pf["g7"])),
            c="g",
            ls="--",
            alpha=alp,
            label="g-g7",
        ...
        ax[0].axvline(
            abs(1 / (prop_elem.proper_elements["g(rev/yr)"][0] - pf["g8"])),
            c="purple",
            ls="--",
            alpha=alp,
            label="g-g8",
        ...
        ax[1].axvline(
            abs(1 / (prop_elem.proper_elements["s(rev/yr)"][0] - pf["s6"])),
            c="goldenrod",
            ls="--",
            alpha=alp,
            label="s-s6",
        ...
        ax[1].axvline(
            abs(1 / (prop_elem.proper_elements["s(rev/yr)"][0] - pf["s7"])),
            c="g",
            ls="--",
            alpha=alp,
            label="s-s7",
        ...
        ax[1].axvline(
            abs(1 / (prop_elem.proper_elements["s(rev/yr)"][0] - pf["s8"])),
            c="purple",
            ls="--",
            alpha=alp,
            label="s-s8",
    ...
    if prop_elem.p_vO:
        ...
        ax[1].axvline(
            abs(1 / prop_elem.proper_elements["s(rev/yr)"][0]), c="blue", ls="--", alpha=alp, label="s"
        )
...
def plot_hkpq(prop_elem):
    ...
    per5 = int(len(hk) / 20)
    ax[0].scatter(
        np.nanmedian(np.imag(hk[per5:-per5])), np.nanmedian(np.real(hk[per5:-per5])), marker="x", c="tab:blue"
    )
    ax[0].scatter(
        np.nanmedian(np.imag(hkn[per5:-per5])),
        np.nanmedian(np.real(hkn[per5:-per5])),
        marker="x",
        c="tab:orange",
    )

    ax[1].scatter(
        np.nanmedian(np.imag(pq[per5:-per5])), np.nanmedian(np.real(pq[per5:-per5])), marker="x", c="tab:blue"
    )
    ax[1].scatter(
        np.nanmedian(np.imag(pqn[per5:-per5])),
        np.nanmedian(np.real(pqn[per5:-per5])),
        marker="x",
        c="tab:orange",
    )
    fig.suptitle("Small Body: " + str(objname), fontsize=16, x=0.52, y=0.94)

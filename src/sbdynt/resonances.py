...
def read_sa_for_resonance(
    ...
    res_string = (
        "$" + "\\" + "phi = " + str(p) + "\\" + "lambda - " + str(q) + "\\" + "lambda_{" + planet + "}"
    )
    if m != 0:
...
def plot_resonance(
    ...
    if des == None or planet == None:
        ...
        try:
            ...
            rflag, a, e, inc, node, aperi, ma, phi, t, res_string = read_sa_for_resonance(
                des=des,
                datadir=datadir,
...
def analyze_res(
    ...
    dpmax = 350.0 * deg_to_rad  # can be lowered to 340 degrees if time sampling isn't high enough
        ...
        for i in range(i_t0, i_tf + 1):
            ...
            if twindow >= dtwindow or np.abs(twindow - dtwindow) < 5:
                ...
                if dphi[j] >= (2.0 * np.pi - dpmax):
                    ...
                    wdphi[nwin] = (2.0 * wphi[-1] + wphi[-2] - 2.0 * wphi[0] - wphi[1]) / 3.0

...
def calc_ML(name,clf=None):
    ...
    try:
        ...
        #print('~/SBDynT_dev/data/NesvornyAsthalf1/'+str(name)+'/archive_init.bin')
        if os.path.isfile('../../SBDynT_dev/data/NesvornyAsthalf1/'+str(name)+'/archive_init.bin'):
            sim_init = rebound.Simulationarchive('../../SBDynT_dev/data/NesvornyAsthalf1/'+str(name)+'/archive_init.bin')[0]
        else:
            #print(os.listdir('../../SBDynT_dev/data/NesvornyAsthalf1/5'))
            print('~/research/SBDynT_dev/data/NesvornyAsthalf1/'+str(name)+'/archive_init.bin is not found')
...
if __name__ == "__main__":
    ...
    with MPIPool() as pool:
        ...
        data = pd.read_csv('~/research/SBDynT_dev/data/data_files/NesvornyAsthalf1.csv')

        #for i in range(len(data)):
        des = np.array(data['Name'].iloc[0:10])
        ...
        c_df['Objname'] = des
        c_df.to_csv('../data/results/NesvornyAsthalf1_classes.csv')
